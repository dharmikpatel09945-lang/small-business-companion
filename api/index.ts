...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { email, password: hashedPassword, displayName, businessName },
    });

    const token = jwt.sign({ userId: user.id, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
    res.json({
      token,
      user: { id: user.id, email: user.email, displayName: user.displayName, businessName: user.businessName },
    });
  } catch (error: any) {
...
app.put('/api/inventory/:id', authenticateToken, async (req: any, res: any) => {
  try {
...
app.get('/api/customers', authenticateToken, async (req: any, res: any) => {
  try {
    const customers = await prisma.customer.findMany({ where: { userId: req.user.userId } });
    res.json(customers);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
// Only start a local server when NOT on Vercel
